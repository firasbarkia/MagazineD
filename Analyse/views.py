...
@user_passes_test(lambda u: is_authenticated(u), login_url="sign")
...
def crypto_data_view(request):
    ...
    filename = (
        "scrapping/coingecko_data.csv"  # Replace with the actual path to your CSV file
    )
    ...
    for fchier in os.listdir(directory):
        ...
        next_24_hours_volume = model.predict(
            [[last_row["Volume"], last_row["Cours d'ouverture"]]]
        )
        next_24_hours_opening = model.predict(
            [[next_24_hours_volume[0], last_row["Cours d'ouverture"]]]
        )

        # Predict the next 24 hours
        next_24_hours_closing = model.predict(
            [[next_24_hours_volume[0], next_24_hours_opening[0]]]
        )
...
def trending(request):
    ...
    for file_name in file_names:
        ...
        table_html = df.to_html(
            index=False, table_id=tableid, classes="table table-striped"
        )
